...
def run(ctx):
    ...
    # incubate for 10 minutes with 1 centrifuge/mix
    ctx.comment('Mixing samples+buffer+beads:')
    ...
    wash_step(wb1, 500, 20, tips2, tips1, '1 Wash Buffer 1')

    wash_step(wb2, 500, 15, tips3, tips2, '2 Wash Buffer 2')

    wash_step(etoh, 600, 15, tips4, tips3, '3 Ethanol Wash', trash_tips=False)
    ...
    for well, tip, tret, x in zip(magsamps, tips5, tips4, x_offset):
        ...
        for _ in range(15):
            m300.dispense(
                50, well.bottom().move(types.Point(x=x, y=0, z=2)))
            m300.aspirate(
                50, well.bottom().move(types.Point(x=x, y=0, z=0.5)))
