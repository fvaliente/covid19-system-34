...
def run(protocol):
    ...
    p300.drop_tip()
    destwells = [well for row in finalPlate.rows() for well in row]
    # add 50uL of GM to A1-A3 and H10-12
    for well in destwells[:3]+destwells[-3:]:
        pick_up(p300)
        p300.aspirate(50, gm.bottom(hts[1]))
        height_tracking(50, 50)
        p300.dispense(50, well)
        p300.blow_out()
        p300.drop_tip()

    samprows = [row for plate in sampTRs for row in plate.rows()]
    sampwells = [well for row in samprows for well in row for i in range(3)]

    for src, dest in zip(sampwells, destwells[3:-3]):
        pick_up(p300)
